...
| `midi_generator/` | Python modules | `storage:midi-generator/midi_generator/` | `/midi-generator/midi_generator/` |
| `config/` | Configuration files | `storage:midi-generator/config/` | `/midi-generator/config/` |
...
* The content of the `midi_generator` and `notebooks` directories is uploaded to the platform storage.
...
This list of hyper-parameters corresponds to the default configuration we provide in `midi_generator/wandb-sweep.yaml` file. See [W&B documentation page](https://docs.wandb.com/library/sweeps) for more details. The name of the sweep file can be modified in `Makefile` or as environment variable `WANDB_SWEEP_FILE`.
